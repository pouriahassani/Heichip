...
#The AXI_blk_mem_gen_0_0 is the name of the Block RAM in the BD.
#This has been tested with a memory range 0K - 1M
#This only supports data width of 32 bits.
#
# BUG FIX: Moved </AddressSpace> outside the bus_blocks loop

proc write_mmi {AXI_blk_mem_gen_0_0} {
	set proj [current_project]
	set filename "${AXI_blk_mem_gen_0_0}.mmi"
	set fileout [open $filename "w"]
	set brams [split [get_cells -hierarchical -filter { PRIMITIVE_TYPE =~ BMEM.bram.* }] " "]
	#isolate all BRAMs identified by AXI_blk_mem_gen_0_0
	set AXI_blk_mem_gen_0_0_bram ""
	for {set i 0} {$i < [llength $brams]} {incr i} {
		if { [regexp -nocase $AXI_blk_mem_gen_0_0 [lindex $brams $i]] } {
			lappend AXI_blk_mem_gen_0_0_bram [lindex $brams $i]
		}
	}
	set proc_found 0	
	...
	set loc_temp [string first $AXI_blk_mem_gen_0_0 $inst_temp]
	...
	set bram_range 0
	for {set i 0} {$i < [llength $AXI_blk_mem_gen_0_0_bram]} {incr i} {
		set bram_type [get_property REF_NAME [get_cells [lindex $AXI_blk_mem_gen_0_0_bram $i]]]
		if {$bram_type == "RAMB36E1"} {
			set bram_range [expr {$bram_range + 4096}]	
		}
	}
	puts $fileout "  <AddressSpace Name=\"$AXI_blk_mem_gen_0_0\" Begin=\"0\" End=\"[expr {$bram_range - 1}]\">"

	set bram [llength $AXI_blk_mem_gen_0_0_bram]
	if {$bram >= 32} {
	...
	for {set b 0} {$b < $bus_blocks} {incr b} {
		...
		for {set i 0} {$i < [llength $sequence]} {incr i} {
			for {set j 0} {$j < [llength $AXI_blk_mem_gen_0_0_bram]} {incr j} {
				set block_start [expr {32768 * $b}]
				set bmm_width [bram_info [lindex $AXI_blk_mem_gen_0_0_bram $j] "bit_lane"]
				set bmm_width [split $bmm_width ":"]
				set bmm_msb [lindex $bmm_width 0]
				set bmm_lsb [lindex $bmm_width 1]
				set bmm_range [bram_info [lindex $AXI_blk_mem_gen_0_0_bram $j] "range"]
				set split_ranges [split $bmm_range ":"]
				set MSB [lindex $sequence $i]
				if {$MSB == $bmm_msb && $block_start == [lindex $split_ranges 0]} {
					set bram_type [get_property REF_NAME [get_cells [lindex $AXI_blk_mem_gen_0_0_bram $j]]]
					set status [get_property STATUS [get_cells [lindex $AXI_blk_mem_gen_0_0_bram $j]]]
																							
					if {$status == "UNPLACED"} {
						set placed "X0Y0"
					} else {
						set placed [get_property LOC [get_cells [lindex $AXI_blk_mem_gen_0_0_bram $j]]]
	...
	# BUG FIX: This was previously inside the for loop above, causing multiple </AddressSpace> tags
