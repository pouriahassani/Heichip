module fpga_ctrl_to_mem_mod (
    ...
    assign data_inout_exmem = data_oe ? data_out : 16'bz;

    assign data_in = data_inout_exmem;
    ...
    always@(*) begin
        ...
        case (cur_state)
            ...
            EX_ADR1: begin
                ...
                data_out = uo_out; // Set address for memory operation
                if (rw == 1'b0) begin // External memory read operation
                    ...
                    data_oe = 1'b1; // Enable output for write
                    data_out = uo_out; // Set data for external memory write
            ...
            EX_RD1: begin
                ready = 1'b1;
                ui_in = data_in; // Capture data from external memory
                nxt_state = EX_RD2;
            end
            ...
            EX_WR2: begin
                data_oe = 1'b0; // Disable output after write
                data_out = uo_out; // Set data for external memory write
